...
def test_single_data_point():
    ...
    y = np.array([0])
    npt.assert_equal(class_aware_trustworthiness(X, X_2d, y, k=1).numpy(), 1.0)
...
def test_one_error_same_class():
    ...
    # Nearest neighbor:
    #    Data Space | Proj Space |
    # 0 |     1     |     2      |  Penalty = 0 because 0 and 2 are of the same class
    # 1 |     2     |     2      |  Penalty = 0
    # 2 |     1     |     1      |  Penalty = 0
    npt.assert_allclose(class_aware_trustworthiness(X, X_2d, y, k=1).numpy(), 1.0)


def test_two_errors():
    # data in a line: 0 -- 1 - 2
    X = np.c_[np.zeros(3), np.array([0.0, 1.5, 2.0]), np.zeros(3)]
    # data in a line, with one swap: 1 - 0 -- 2
    X_2d = np.c_[np.zeros(3), np.array([1.9, 0.0, 2.1])]
    # Nearest neighbor:
    #    Data Space | Proj Space |
    # 0 |     1     |     2      |  Penalty = 1
    # 1 |     2     |     0      |  Penalty = 1
    ...
    # All penalties apply because all classes are different
    y = np.array([0, 1, 2])
    npt.assert_allclose(class_aware_trustworthiness(X, X_2d, y, k=1).numpy(), 0.0)
